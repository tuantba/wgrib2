...
/** Number of grid points in x and y directions (assumed available in wgrib2). */
extern int nx, ny;
 ...
 * 
 * The format is: 
 *      When WxNum > 0: i,j,"WxLabel(data[j])"
 *      Otherwise: "time0","time1","field","level",longitude,latitude,grid-value
 * 
 ...
 * For a forecast for a specific time, then time1 will be the time that the forecast is valid. For a 
 * forecast of a temporal average or accumulation, time1 will be the end of the average or accumulation.
 *
 ...
 * the high-resolution models can still generate huge files. The next technique is to undefine the
 * grid points that you don't want. For example, you are only interested in Hawaii (its 0C outside 
 * as I write this). Then you can use the -undefine option to set the grid points outside of the 
 * Hawaii domain to undefined. Since -csv_long doesn't print undefined grid points, the CSV 
 ...
 * with the CSV file. After all, everybody can read a CSV file. Sure there is a little overhead of reading 
